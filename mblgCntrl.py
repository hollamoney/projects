...
while True:
    ...
    def hello(name):
        ...
        if len(sorted_contours) >= 2 and int(
                find_contours(sorted_contours)[0] + find_contours(sorted_contours)[1]) >= 1000:
            dt_now = datetime.datetime.now()
            print(int(find_contours(sorted_contours)[0] + find_contours(sorted_contours)[1]))
            print(dt_now)
            con = sqlite3.connect("project123.db", check_same_thread=False)
            cursor = con.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS Data (alan INT,tarih TEXT)")
            cursor.execute("INSERT INTO Data VALUES(?,?)",
                           ((find_contours(sorted_contours)[0] + find_contours(sorted_contours)[1],
                             str(dt_now))))
            con.commit()
        else:
            dt_now = datetime.datetime.now()
